...
class MinerUOnlineClient:
    ...
    def process(self, file_path, force_ocr=False):
        """
        ...
        Args:
            file_path: PDF 文件路径
            force_ocr: 是否强制 OCR 识别
        """
        original_file = Path(file_path)
        
        # 步骤1: 申请上传链接
        st.toast("1. 申请上传链接...", icon="🔗")
        upload_url, batch_id = self._get_upload_url(original_file.name, force_ocr)  # ⭐ 传递参数
    ...
    def _get_upload_url(self, filename, force_ocr=False):  # ⭐ 添加 force_ocr 参数
        """申请文件上传链接"""
...
def main():
    ...
    with st.sidebar:
        ...
        force_ocr = st.checkbox(
            "🔍 强制 OCR 识别",
            value=False,
            help="开启后对所有页面进行 OCR 文字识别，适用于扫描版 PDF 或图片型文档，处理时间会更长"
        )
        st.session_state.force_ocr = force_ocr  # 保存到 session_state
